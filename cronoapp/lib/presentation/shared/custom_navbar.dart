  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    void resetValues(WidgetRef ref){
      final current = PersonConfig(
          ...
          ref.read(secsBreak.notifier).state = current.breakDurationTime.split(":")[1];
          ref.read(minsBreak.notifier).state = current.breakDurationTime.split(":")[0];
          ref.read(secsExercise.notifier).state = current.exerciseDurationTime.split(":")[1];
          ref.read(minsExercise.notifier).state = current.exerciseDurationTime.split(":")[0];
